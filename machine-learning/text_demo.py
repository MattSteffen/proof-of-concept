"""
# Text Classification Baseline Methods: When and Why to Choose Each Approach

When starting any text classification project, establishing strong baselines is crucial for understanding your problem and measuring progress. This guide demonstrates several fundamental methods, each serving specific purposes in your machine learning workflow.

## Method Overview

### 1. **Naive Baselines**
**What it is:** Simple heuristics like predicting the most frequent class or random guessing.

**When to use:** Always start here to establish minimum performance thresholds.

**Why choose this:** Provides context for evaluating more sophisticated models. If your complex model barely beats random guessing, you likely have a problem with your data or approach.

### 2. **TF-IDF + Logistic Regression**
**What it is:** Term Frequency-Inverse Document Frequency vectorization combined with logistic regression classification.

**When to use:** As your first "real" baseline for any text classification task, especially when you need interpretable results or have limited computational resources.

**Why choose this:** 
- Fast to train and predict
- Handles high-dimensional sparse data exceptionally well
- Provides interpretable feature weights (you can see which words drive predictions)
- Surprisingly competitive performance across many domains
- Requires minimal preprocessing
- Excellent starting point before exploring deep learning approaches

### 3. **N-gram Enhancement**
**What it is:** Extending TF-IDF to capture phrase-level patterns (bigrams, trigrams) rather than just individual words.

**When to use:** When context matters significantly in your domain (like sentiment analysis where "not good" is very different from "good").

**Why choose this:** 
- Captures local context and phrase-level meaning
- Often provides meaningful improvements over unigrams alone
- Still maintains interpretability and speed
- Particularly effective for sentiment analysis, spam detection, and tasks where negation matters

### 4. **Hyperparameter Tuning**
**What it is:** Systematic optimization of model parameters like vocabulary size, regularization strength, and n-gram ranges.

**When to use:** After establishing your basic approach, to squeeze out maximum performance from your baseline.

**Why choose this:** 
- Ensures you're getting the best possible performance from your chosen method
- Provides a strong benchmark for evaluating more complex approaches
- Helps understand the sensitivity of your approach to different parameters
- Often yields 2-5% accuracy improvements with minimal additional complexity

## Decision Framework

**Start with TF-IDF + Logistic Regression when you:**
- Need a quick, interpretable baseline
- Have limited computational resources
- Want to understand which features drive predictions
- Are working with traditional NLP tasks (sentiment, spam detection, topic classification)

**Move to deep learning approaches only after:**
- Your TF-IDF baseline is solid and well-tuned
- You have sufficient data (typically 10K+ examples)
- The problem complexity justifies the additional overhead
- You've exhausted improvements from feature engineering and hyperparameter tuning

The beauty of this baseline approach is its simplicity and effectiveness. Many production systems still rely on these methods because they're fast, interpretable, and surprisingly powerful when properly tuned.
"""
...
def load_and_explore_data():
    ...
    reviews_text = [
        doc.decode("utf-8") for doc in reviews_train.data + reviews_test.data
    ]
...
tuned_model, tuned_accuracy = hyperparameter_tuning(X_train, X_test, y_train, y_test)
