"""
# Computer Vision Baseline Methods: When and Why to Choose Each Approach

Computer vision tasks require a different baseline strategy than text classification, primarily due to the high dimensionality and spatial nature of image data. This guide demonstrates several fundamental approaches, each with distinct advantages for establishing strong baselines in image classification projects.

## Method Overview

### 1. **Naive Baselines**
**What it is:** Simple heuristics like predicting the most frequent class or random guessing across all image classes.

**When to use:** Always start here to establish minimum performance thresholds for any computer vision task.

**Why choose this:** Provides essential context for evaluating more sophisticated models. In balanced datasets like Fashion-MNIST, random guessing gives you 10% accuracy (1/10 classes), so any real model should significantly exceed this.

### 2. **Traditional ML + Hand-crafted Features**
**What it is:** Manually designed features (pixel statistics, edge detection, texture descriptors) combined with classical machine learning algorithms.

**When to use:** When you need highly interpretable results, have domain expertise to design good features, or computational resources are extremely limited.

**Why choose this:** 
- Completely interpretable - you know exactly what features drive predictions
- Fast training and inference
- Works with very small datasets
- Good for understanding your problem domain
- Useful baseline before investing in deep learning approaches

### 3. **Simple CNN from Scratch**
**What it is:** Training a basic convolutional neural network architecture from randomly initialized weights.

**When to use:** Primarily for comparison purposes or when you have a very unique domain where pre-trained features might not transfer well.

**Why choose this:** 
- Shows what's achievable with basic deep learning
- Helps you understand if your problem needs the complexity of CNNs
- Useful for gauging the value of transfer learning (by comparison)
- Educational value for understanding CNN architectures

### 4. **Transfer Learning (Fine-tuning)**
**What it is:** Taking a pre-trained CNN (like ResNet50 trained on ImageNet) and fine-tuning it for your specific task.

**When to use:** This should be your primary baseline for most computer vision tasks, especially when you have moderate amounts of data (1K-100K images).

**Why choose this:** 
- Leverages features learned from millions of images (ImageNet)
- Works remarkably well even across different domains (natural images → medical images → satellite imagery)
- Much faster than training from scratch
- Requires less data than training from scratch
- Consistently provides strong performance across diverse vision tasks
- Good balance of performance and computational efficiency

### 5. **Feature Extraction + Traditional ML**
**What it is:** Using a pre-trained CNN as a fixed feature extractor, then training simple classifiers (LogReg, Random Forest) on the extracted features.

**When to use:** When you need fast inference, have limited computational resources, or want to combine the power of deep features with the interpretability of traditional ML.

**Why choose this:** 
- Combines the best of both worlds: powerful CNN features + fast traditional ML
- Much faster training than fine-tuning
- Faster inference than full CNN models
- Often achieves 90-95% of fine-tuning performance with fraction of the computational cost
- Great for production systems where speed matters
- Easier to debug and interpret than end-to-end deep learning

## Decision Framework

**Start with Transfer Learning (Fine-tuning) when you:**
- Have a typical computer vision classification task
- Have at least 1,000+ labeled images per class
- Want the best possible performance
- Have access to GPU resources
- Are willing to accept some "black box" behavior

**Choose Feature Extraction + Traditional ML when you:**
- Need fast training and inference
- Have limited computational resources
- Want to maintain some interpretability
- Are building production systems where speed/resources matter
- Want nearly the same performance as fine-tuning with less complexity

**Use Traditional ML + Hand-crafted Features when you:**
- Need complete interpretability and explainability
- Have very small datasets (< 100 images per class)
- Have domain expertise to design good features
- Are in regulated industries requiring explainable models
- Want to understand your problem better before using deep learning

**Only consider CNN from Scratch when you:**
- Have a very unique domain where ImageNet features don't transfer
- Have massive datasets (100K+ images) and computational resources
- Need to understand if the problem requires CNN-level complexity
- Are doing research on new architectures

## Key Insights

The progression shown in this demonstration reveals a crucial insight: **transfer learning with pre-trained CNNs is almost always your best starting point** for computer vision baselines. The features learned on ImageNet are surprisingly general and transfer well to diverse domains.

The feature extraction approach is particularly valuable because it gives you 90-95% of the performance of full fine-tuning while being:
- 10x faster to train
- 5x faster for inference  
- Much easier to debug and understand
- More suitable for production deployment

This makes it an excellent "best of both worlds" baseline that combines the representational power of deep learning with the speed and interpretability of traditional machine learning.
"""
